...
content += `
<h3 class="wiki-heading"><strong>편집 요청</strong></h3>
...
content += `
<h3 class="wiki-heading"><strong>토론</strong></h3>
<div class=wiki-heading-content>
<ul class=wiki-list>
`;
...
? '[' + rs['hider'] + '에 의해 숨겨진 글입니다.]<div class="text-line-break" style="margin: 25px 0px 0px -10px; display:block"><div class="line"></div></div><div class="hidden-content" style="display:none">' + await markdown(req, rs['content'], 1) + '</div>'
...
<h3 class="wiki-heading"><strong>새 주제 생성</strong></h3>
...
<div class="btns">
<button id="createBtn" class="btn btn-primary" style="width: 8rem; border-radius: 0px; font: 13.5px Arial; padding: 3px 10.5px">전송</button>
</div>
...
<h2 class=wiki-heading style="cursor: pointer;"><strong>댓글 달기</strong></h2>
`;
...
<button id=changeBtn class="btn btn-secondary" style="border-radius:0px; background-color:#fff; padding: 3px 10.5px; font:13.5px Arial">변경</button>
</form>
`;
}
...
<div class=btns>
<button type=submit class="btn btn-primary" style="padding: 4px 10.5px; float: right; font: 13.5px Arial; border-radius:0px; width:120px"${['close', 'pause'].includes(status) ? ' disabled' : ''}>전송</button>
</div>
</form>
`;
...
<span class=num>
<a id="${rs.id}">#${rs.id}</a>&nbsp;
</span>${ip_pas(rs.username_disbled, rs.ismember, 1).replace('<a ', rs.isadmin == '1' ? '<a style="font-weight: bold;" ' : '<a ')}${rs['ismember'] == 'author' &&
await userblocked(rs.username) ? `<del><a href="/w/사용자:${rs.username}" style="color: grey">${rs.username}</a></del>`.replace('<a ', rs.isadmin == '1' ? '<a style="font-weight: bold;" ' : '<a ') : `<a href="/w/사용자:${rs.username}">${rs.username}</a>`.replace('<a ', rs.isadmin == '1' ? '<a style="font-weight: bold;" ' : '<a ')
}${rs.ismember == 'ip' && await ipblocked(rs.username)
? ` <small>(${'차단됨'})</small>` : ''}
